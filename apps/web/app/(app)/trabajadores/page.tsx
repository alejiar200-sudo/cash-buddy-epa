      ...
      {adding && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={() => setAdding(false)}>
          <div className="glass-strong rounded-3xl p-6 w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
            ...
            <button onClick={() => { if (name.trim()) { void addWorker({ name: name.trim(), role, active: true }); setName(""); setAdding(false); } }} className="mt-4 w-full bg-primary text-primary-foreground font-bold py-3 rounded-xl shadow-cash">Agregar</button>
          </div>
        </div>
      )}
    </div>
