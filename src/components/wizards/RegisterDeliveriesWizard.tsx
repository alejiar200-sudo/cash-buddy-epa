...
export function RegisterDeliveriesWizard({ open, onOpenChange, date, workerId }: Props) {
  ...
  function submit() {
    ...
    if (!worker) return;
    if (cash > 0) {
      addMovement(date, { category: 1, type: "ingreso", medium: "cash", amount: cash, workerId, description: `Domicilios efectivo - ${worker.name}`, status: "pending" });
    }
    if (bank > 0) {
      addMovement(date, { category: 2, type: "ingreso", medium: "bank", amount: bank, workerId, description: `Domicilios banco - ${worker.name}`, status: "pending" });
    }
