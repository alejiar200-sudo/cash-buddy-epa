...
export function WelcomeWizard() {
  const { state, setState, addWorker } = useStore();
  ...
  function finish() {
    setState((s) => ({
      ...s,
      settings: { ...s.settings, companyName: name, initialCash: cash, initialBank: bank, setupComplete: true },
    }));
  }
        ...
        {step === 4 && (
          <>
            ...
            <div className="mt-4 max-h-64 overflow-auto space-y-1.5">
              {state.workers.filter(w => w.role === "domiciliario").map(w => (
                <div key={w.id} className="flex items-center gap-3 glass rounded-xl p-2.5">
                  ...
                  <button onClick={() => setState(s => ({ ...s, workers: s.workers.filter(x => x.id !== w.id) }))} className="p-2 text-danger hover:bg-danger-soft rounded-lg">
            ...
            <div className="flex gap-2 mt-3">
              ...
              <button onClick={() => { if (newWorker.trim()) { addWorker({ name: newWorker.trim(), role: "domiciliario", active: true }); setNewWorker(""); } }} className="bg-primary text-primary-foreground px-4 rounded-xl font-bold flex items-center gap-1">
