...
export function CourierPaidWizard({ open, onOpenChange, date, workerId }: Props) {
  ...
  function submit() {
    ...
    pendings.forEach((m) => updateMovement(date, m.id, { status: "confirmed" }));

    // 2. Register base return as cash ingreso (cat 5)
    const baseToReturn = Math.min(amount, status.baseGiven - status.baseReturned);
    if (baseToReturn > 0) {
      addMovement(date, { category: 5, type: "ingreso", medium: "cash", amount: baseToReturn, workerId, description: `Devolución base - ${worker.name}`, status: "confirmed" });
    }
